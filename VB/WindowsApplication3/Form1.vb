...
'INSTANT VB NOTE: The variable text was renamed since Visual Basic does not handle local variables named the same as class members well:
				Dim text_Renamed As String = item.GetDisplayText(values(item), False)
				rect = CalcSummaryRect(text_Renamed, e, view.Columns(item.FieldName))
				e.Appearance.DrawString(e.Cache, text_Renamed, rect)
			Next item
		End Sub
...
'INSTANT VB NOTE: The variable width was renamed since Visual Basic does not handle local variables named the same as class members well:
			Dim width_Renamed As Integer = Convert.ToInt32(sz.Width) + 1
			If Not gridInfo.ViewRects.FixedLeft.IsEmpty Then
				Dim fixedLeftRight As Integer = gridInfo.ViewRects.FixedLeft.Right
				Dim marginLeft As Integer = result.Right - width_Renamed - fixedLeftRight
			...
			If Not gridInfo.ViewRects.FixedRight.IsEmpty Then
				...
				End If
			End If
			result = FixLeftEdge(width_Renamed, result)
